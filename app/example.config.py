...
def load_settings(base_dir: Path) -> Settings:
    ...
    return Settings(
        app_title=os.getenv("APP_TITLE", "Akuvox Warehouse").strip() or "Akuvox Warehouse",
        ...
        dev_login_email=os.getenv("DEV_LOGIN_EMAIL", "admin@example.com").strip(),
        allowlist_domain=os.getenv("ALLOWLIST_DOMAIN", "example.com").strip().lower(),
        ...
        dev_login_password=os.getenv("DEV_LOGIN_PASSWORD", "change-me").strip(),
        akuvox_scheme=os.getenv("AKUVOX_SCHEME", "http").strip().lower(),
        akuvox_ip=os.getenv("AKUVOX_IP", "127.0.0.1").strip(),
        akuvox_display_name=os.getenv("AKUVOX_DISPLAY_NAME", "Akuvox Device").strip() or "Akuvox Device",
        akuvox_username=os.getenv("AKUVOX_USERNAME", "admin").strip(),
        akuvox_password=os.getenv("AKUVOX_PASSWORD", "admin").strip(),
